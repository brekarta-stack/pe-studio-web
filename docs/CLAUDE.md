...
**★ 게이트 우회 금지 (2026-07-28 비용감사로 추가된 불변 원칙)** — 모든 LLM 호출은 **프록시 HTTP 엔드포인트**를 지나야 한다.
`import litellm`·`google.generativeai`·`anthropic` 등 **SDK/라이브러리 직접 호출 금지**.
- **근거(실측)**: 미니 `proactive-nudge`가 litellm 파이썬 라이브러리로 Gemini를 직접 불러 **프록시 DB에 한 줄도 안 남았다**.
  게이트웨이 안 전체 지출 $0.0366 / 이 잡 하나 **월 ~$25(112배)**. 예산캡·집계·폴백이 전부 무력.
- **왜 원칙인가**: "단일 게이트로 비용을 통제한다"는 C-2의 전제가 라이브러리 직접 호출 한 줄로 무너진다. 미니 잔재 문제가 아니라 **구조 문제**다.
- **적용**: 신규 코드 리뷰 시 SDK import 여부 확인. 부득이한 예외는 **매매 엔진의 LiteLLM 우회 폴백(GD-3)** 하나뿐이며, 그것도 문서에 명시된 경로만.
...
- **★ 추론형 모델은 저가 티어가 아니다 (3연속 실측)**: ① 로컬 35B — `think:false` 안 주면 사고에 토큰 소진, 무제한이면 7092토큰(85초) vs 82토큰(1초), **86배**.
  ② kimi-k2.6 — max_tokens 작으면 reasoning으로 다 쓰고 **content 빈 문자열**. ③ **gemini-flash — "저가라 허용"이라 적어둔 판단이 무효**:
  출력 2,890토큰 중 **2,708이 reasoning**, 쓸모 있는 JSON은 182토큰. 콜당 $0.0275, **비용의 89%가 사고 과정**.
  → **모델 선정 시 "단가"가 아니라 "reasoning 포함 실측 콜당 비용"으로 판단.** 저가 티어 배정 전 1콜 실측 필수.
