...
- **문서의 "완료" 주장을 의심하라.** 07-23의 "스키마 로드됨"이 거짓이었다. 실행해서 확인할 것.
  (07-28 재확인: 이 HANDOFF의 "n8n 8종 전부 inactive"도 **거짓**이었다 — 아침 브리핑이 active=t로 실제 발송됐다.)
- **★ 판정 기준은 "거부 문구"가 아니라 "산출물 유무"** (07-28 비용감사): `sns-daily`가 18일째 산출 0인데도
  CLI가 파일을 안 쓰고 "OK"를 출력해 **정상 종료로 보였다**. 원인은 `--print`가 아니라 **워크스페이스 미신뢰로 permissions.allow 82개가 통째 무시**.
  게다가 호출부가 출력을 버려서 **로그 기반 탐지는 원리적으로 불가능**했다. → 감사 스크립트는 로그가 아니라 **결과물 존재**로 판정하고,
  간접 참조는 **2단계 이상 추적**할 것(1단계만 봐서 "피해는 bd-daily 단독"이라는 오탐이 났다).
